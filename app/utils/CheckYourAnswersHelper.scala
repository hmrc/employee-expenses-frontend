...
class CheckYourAnswersHelper(userAnswers: UserAnswers)(implicit messages: Messages) {
  ...
  def removeFRECode: Option[AnswerRow] = userAnswers.get(RemoveFRECodePage).map { x =>
    AnswerRow(
      ...
      answer = messages(
        s"taxYearSelection.$x",
        TaxYearSelection.getTaxYear(x).toString,
        (TaxYearSelection.getTaxYear(x) + 1).toString
      ),
  ...
  def alreadyClaimingFREDifferentAmounts: Option[AnswerRow] =
    userAnswers.get(AlreadyClaimingFREDifferentAmountsPage).map { x =>
      AnswerRow(
  ...
  def alreadyClaimingFRESameAmount: Option[AnswerRow] = userAnswers.get(AlreadyClaimingFRESameAmountPage).map { x =>
    AnswerRow(
  ...
  def sameEmployerContributionAllYears: Option[AnswerRow] =
    userAnswers.get(SameEmployerContributionAllYearsPage).map { x =>
      AnswerRow(
  ...
  def changeWhichTaxYears: Option[AnswerRow] = userAnswers.get(ChangeWhichTaxYearsPage).map { taxYears =>
    AnswerRow(
  ...
  def industryType: Option[AnswerRow] =
    userAnswers.get(FirstIndustryOptionsPage) match {
      ...
      case Some(TransportAndDistribution) =>
        industryAnswerRow(s"firstIndustryOptions.${TransportAndDistribution.toString}")
  ...
  def secondaryIndustryList: Option[AnswerRow] =
    userAnswers.get(SecondIndustryOptionsPage) match {
      ...
      case Some(ManufacturingWarehousing) =>
        industryAnswerRow(s"secondIndustryOptions.${ManufacturingWarehousing.toString}")
  ...
  def fifthIndustryList: Option[AnswerRow] =
    userAnswers.get(FifthIndustryOptionsPage) match {
      ...
      case Some(FifthIndustryOptions.NoneOfAbove) =>
        industryAnswerRow(s"fifthIndustryOptions.${FifthIndustryOptions.NoneOfAbove.toString}")
      case _ => industryAnswerRow("default rate")
    }

  def employerContribution: Option[AnswerRow] =
    userAnswers.get(EmployerContributionPage).map { employerContributionOption =>
      AnswerRow(
  ...
  def expensesEmployerPaid: Option[AnswerRow] = userAnswers.get(EmployerContributionPage) match {
    case Some(EmployerContribution.YesEmployerContribution) =>
      userAnswers.get(ExpensesEmployerPaidPage).map { x =>
        AnswerRow(
  ...
  def taxYearSelection: Option[AnswerRow] = userAnswers.get(TaxYearSelectionPage).map { taxYears =>
    AnswerRow(
