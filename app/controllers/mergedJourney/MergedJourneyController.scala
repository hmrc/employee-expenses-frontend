...
@Singleton
class MergedJourneyController @Inject() (
      ...
      if (appConfig.mergedJourneyEnabled) {
        request.identifier match {
          case id: Authed if List(wfh, psubs, fre).count(_ == true) > 1 =>
            for {
              wfhClaimedAlready <-
                if (wfh) employeeWfhExpensesConnector.checkIfAllYearsClaimed(hc) else Future.successful(false)
              _ <-
                if (!wfhClaimedAlready) {
              ...
              url =
                ...
                else controllers.mergedJourney.routes.ClaimYourExpensesController.show.url
            } yield Redirect(url)
          case _ =>
            logger.warn(
              s"[MergedJourneyController][setupMergedJourney] Invalid journey flags provided," +
