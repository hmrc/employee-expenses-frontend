...
class ConstructionOccupationList1Controller @Inject()(
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(ConstructionOccupationList1Page) match {
        case None => form
        ...
        value => {
          for {
            updatedAnswers <- if (value) {
              Future.fromTry(request.userAnswers.set(ConstructionOccupationList1Page, value)
                .flatMap(_.set(ClaimAmount, ClaimAmounts.Construction.list1))
              )
            } else {
              Future.fromTry(request.userAnswers.set(ConstructionOccupationList1Page, value))
            }
