  ...
  "TaiService" must {
    ...
    "getFREAmount" must {
      ...
      "return a Future[Seq[(Option[FlatRateExpenseAmounts], TaiTaxYear)]]" in {
        ...
        whenReady(result) {
          _ mustBe Seq(FlatRateExpenseAmounts(Some(FlatRateExpense(100)), TaiTaxYear()))
        }
      }
    }
    ...
    "freResponse" must {
      ...
      "return FREAllYearsAllAmountsSameAsClaimAmount grossAmount is the same as claimAmount for all tax years" in {
        ...
        whenReady(result) {
          _ mustBe FREAllYearsAllAmountsSameAsClaimAmount
        }
      }
      ...
      "return FRESomeYears for all other combinations of freAmount (empty, 0, > 0)" in {
        ...
        val result = taiService.freResponse(
          Seq(CurrentYear, CurrentYearMinus1, CurrentYearMinus2, CurrentYearMinus3),
          fakeNino,
          claimAmount = 200
        )
    ...
    "currentPrimaryEmployer" must {
      ...
      "return an employer when available" in {
        ...
        whenReady(result) {
          _.head.name mustBe "HMRC LongBenton"
        }
      }
    }
  }
