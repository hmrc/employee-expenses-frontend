...
class HeatingOccupationListControllerSpec
    extends SpecBase
    with ScalaFutures
    with MockitoSugar
  ...
  "HeatingOccupationList Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers().set(HeatingOccupationListPage, true).success.value
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val application =
        applicationBuilder(userAnswers = Some(emptyUserAnswers))
          ...
          .overrides(bind[Navigator].qualifiedWith(NavConstant.heating).toInstance(new FakeNavigator(onwardRoute)))
          .build()
    ...
    "save 'list' to ClaimAmount when 'Yes' is selected" in {
      ...
      val userAnswers2 = userAnswers
        .set(ClaimAmount, ClaimAmounts.Heating.list)
        .success
        .value
        .set(HeatingOccupationListPage, true)
        .success
        .value
    ...
    "save 'allOther' to ClaimAmount when 'No' is selected" in {
      ...
      val userAnswers2 = userAnswers
        .set(ClaimAmount, ClaimAmounts.Heating.allOther)
        .success
        .value
        .set(HeatingOccupationListPage, false)
        .success
        .value
