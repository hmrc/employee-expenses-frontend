...
class AncillaryEngineeringWhichTradeControllerSpec
    extends SpecBase
    with ScalaFutures
    with MockitoSugar
  ...
  lazy val ancillaryEngineeringWhichTradeRoute: String =
    routes.AncillaryEngineeringWhichTradeController.onPageLoad(NormalMode).url
  ...
  "AncillaryEngineeringWhichTrade Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers =
        UserAnswers().set(AncillaryEngineeringWhichTradePage, AncillaryEngineeringWhichTrade.values.head).success.value
    ...
    for (trade <- AncillaryEngineeringWhichTrade.values if trade != AncillaryEngineeringWhichTrade.Or) {
      ...
      s"save $claimAmount to ClaimAmount when '$trade' is selected" in {
        ...
        val request =
          FakeRequest(POST, ancillaryEngineeringWhichTradeRoute).withFormUrlEncodedBody(("value", trade.toString))
        ...
        val userAnswers2 = userAnswers
          ...
          .success
          .value
          .set(AncillaryEngineeringWhichTradePage, trade)
          .success
          .value
