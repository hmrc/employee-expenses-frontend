...
class ClaimAmountViewSpec extends ViewBehaviours {
  ...
  "ClaimAmount view" must {
    ...
    def claimAmountsAndRates(deduction: Option[Int] = None) = StandardRate(
      ...
      calculatedBasicRate =
        claimAmountService.calculateTax(frontendAppConfig.taxPercentageBasicRate, claimAmount - deduction.getOrElse(0)),
      calculatedHigherRate =
        claimAmountService.calculateTax(frontendAppConfig.taxPercentageHigherRate, claimAmount - deduction.getOrElse(0))
    )
    ...
    def scottishClaimAmountsAndRates(deduction: Option[Int] = None) = ScottishRate(
      ...
      calculatedStarterRate = claimAmountService
        .calculateTax(frontendAppConfig.taxPercentageScotlandStarterRate, claimAmount - deduction.getOrElse(0)),
      calculatedBasicRate = claimAmountService
        .calculateTax(frontendAppConfig.taxPercentageScotlandBasicRate, claimAmount - deduction.getOrElse(0)),
      calculatedIntermediateRate = claimAmountService
        .calculateTax(frontendAppConfig.taxPercentageScotlandIntermediateRate, claimAmount - deduction.getOrElse(0)),
      calculatedHigherRate = claimAmountService
        .calculateTax(frontendAppConfig.taxPercentageScotlandHigherRate, claimAmount - deduction.getOrElse(0)),
      calculatedAdvancedRate = claimAmountService
        .calculateTax(frontendAppConfig.taxPercentageScotlandAdvancedRate, claimAmount - deduction.getOrElse(0)),
      calculatedTopRate = claimAmountService.calculateTax(
        frontendAppConfig.taxPercentageScotlandTopRate,
        claimAmount - deduction.getOrElse(0)
      )
    )
